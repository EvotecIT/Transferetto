...
### __AllParameterSets
```powershell
Receive-SCPDirectory -ScpClient <TransferettoScpSession> -RemotePath <string> -LocalPath <string> [-ShowProgress] [-ProgressIntervalBytes <long>] [<CommonParameters>]
```
...
### EXAMPLE 1
```powershell
Receive-SCPDirectory -ScpClient 'Value' -RemotePath 'C:\Path' -LocalPath 'C:\Path'
```
