...
### __AllParameterSets
```powershell
Receive-TransferItem [-Endpoint] <ITransferEndpoint> [-Path] <string> [-LocalPath] <string> [-Force] [-ShowProgress] [<CommonParameters>]
```
...
### EXAMPLE 1
```powershell
Receive-TransferItem -Path 'C:\Path'
```
...
### -Force
Gets or sets whether an existing local file is replaced.
...
### -LocalPath
Gets or sets the local destination file.
