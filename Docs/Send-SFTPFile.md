...
### __AllParameterSets
```powershell
Send-SFTPFile -SftpClient <TransferettoSftpSession> [-RemotePath <string>] [-LocalPath <string>] [-AllowOverride] [-ShowProgress] [-ProgressIntervalBytes <long>] [<CommonParameters>]
```
...
### EXAMPLE 1
```powershell
Send-SFTPFile -SftpClient 'Value'
```
